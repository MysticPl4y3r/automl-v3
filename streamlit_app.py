...
def download_data(button,data,save_file):
    with open(data, 'rb') as f:
        df = f.read()
    st.download_button(button, data=df, file_name=save_file)
...
def Upload_data(id):
    ...
    uploaded_file = st.file_uploader("Choose a supported file", type=['csv','parquet'],key=id)
    if uploaded_file:
        save_folder = "./data"
...
def View_data(data):
    ...
    st.divider()
    st.subheader("Data Addition")
    st.caption("Add rows or columns if needed")
    Transform_Data()

def Transform_Data():
    ops = st.selectbox(
    "Select the operation:",
    ("Add new row", "Insert New Column"),
    index=None,
    placeholder="- Select -",
    )
    with st.container(border=True):
        if ops=="Add new row":
            rows=Upload_data("2")
            if rows!=None:
                if (len(st.session_state.data.columns)==len(rows.columns)):
                    st.session_state.data=pd.concat([st.session_state.data,rows],ignore_index=True)
                    st.dataframe(st.session_state.data)
                else:
                    st.warning(f"No. of columns are mismatched ({len(st.session_state.data.columns)},{len(rows.columns)})")
        elif ops=="Insert New Column":
            column=Upload_data("3")
            if column!=None:
                if (st.session_state.data.shape[0]==column.shape[0]):
                    st.session_state.data.insert(loc=0,column=column.columns[0],value=column)
                    st.dataframe(st.session_state.data)
                else:
                    st.warning(f"No. of rows are mismatched ({st.session_state.data.shape[0]},{column.shape[0]})")
        else:
...
def predictions():
    st.header("Features")
    tab1,tab2=st.tabs(["Single Value Predition","Multi Value Prediction"])
    with tab1:
        co1=[]
        col1, col2 = st.columns(2)
    ...
    with tab2:
        upload_file = st.file_uploader("Choose a file containing multiple values:", type=['csv','parquet'])
        if upload_file:
            save_folder = "./predict"
            save_path = Path(save_folder, upload_file.name)
            st.session_state.filename=upload_file.name
            with open(save_path, 'wb') as w:
                w.write(upload_file.getvalue())
            if save_path.exists():
                st.info(f'File {upload_file.name} is uploaded!')
            if str(save_path).endswith(".csv"):
                x_test = pd.read_csv(save_path)
            else:
                x_test = pd.read_parquet(save_path)
            
            x_test=x_test[st.session_state.X_train.columns]
            x_test.dropna(inplace=True)
            le0=LabelEncoder()
            dta=pd.DataFrame(data=x_test,columns=x_test.columns)
            for column in x_test.columns:
                if x_test[column].dtypes=="object":
                    x_test[column]=le0.fit_transform(x_test[column])
            
            if st.button("Generate Prediction"):
                if st.session_state.y_labels is not None:
                    le=LabelEncoder()
                    le.classes_=st.session_state.y_labels
                    res = pd.Series(le.inverse_transform(predict(np.array(x_test))))
                else:
                    res = pd.Series(predict(np.array(x_test)))
                st.success("Prediction done!")

                res.rename(st.session_state.target,inplace=True)
                dta=pd.concat([dta,res],axis=1)
        
                st.write(dta)
                cols1,cols2=st.columns(2)
                with cols1:
                    res.to_csv("./predict/prediction.csv",index=False)
                    download_data("Download Prediction Data Only","./predict/prediction.csv","prediction.csv")
                with cols2:
                    dta.to_csv("./predict/data_prediction.csv",index=False)
                    download_data("Download Full Dataset along with Prediction","./predict/data_prediction.csv","data_prediction.csv")
...
if app == "Load Data":
    ...
    st.session_state.data = Upload_data("1")
